...
# 
# ![Website detection Example](https://i.imgur.com/mOG3u3Z.png)
# 
...
# # Step 3: Train Our Custom YOLOv5l6 model
# 
...
get_ipython().system('python train.py --img 640 --batch 16 --epochs 150 --data {dataset.location}/data.yaml --weights yolov5l6.pt --cache')
...
#!python detect.py --weights runs/train/exp/weights/best.pt --img 640 --conf-thres 0.25 --line-thickness 1 --max-det 30 --source {dataset.location}/test/images
...
get_ipython().system('zip -r /content/exp13.zip /content/yolov5/runs/detect/exp3/')
...
from google.colab import files
files.download('/content/exp13.zip')


# In[8]:


get_ipython().system('python val.py --img 640 --data {dataset.location}/data.yaml --weights runs/train/exp/weights/best.pt --save-txt --save-conf --task test')
